...
func (d oracle) HasIndex(tableName string, indexName string) bool {
	var count int
	d.db.QueryRowContext(d.context,"SELECT COUNT(1) FROM USER_INDEXES WHERE TABLE_NAME = :1 AND INDEX_NAME = :2", strings.ToUpper(tableName), strings.ToUpper(indexName)).Scan(&count)
	return count > 0
}

func (d oracle) RemoveIndex(tableName string, indexName string) error {
	_, err := d.db.ExecContext(d.context,fmt.Sprintf("DROP INDEX %v", indexName))
	return err
}

func (d oracle) HasForeignKey(tableName string, foreignKeyName string) bool {
	var count int
	d.db.QueryRowContext(d.context,"SELECT COUNT(1) FROM USER_CONSTRAINTS WHERE CONSTRAINT_TYPE = 'R' AND TABLE_NAME = :1 AND CONSTRAINT_NAME = :2", strings.ToUpper(tableName), strings.ToUpper(foreignKeyName)).Scan(&count)
	return count > 0
}

func (d oracle) HasTable(tableName string) bool {
	var count int
	d.db.QueryRowContext(d.context,"SELECT COUNT(1) FROM USER_TABLES WHERE TABLE_NAME = :1", strings.ToUpper(tableName)).Scan(&count)
	return count > 0
}

func (d oracle) HasColumn(tableName string, columnName string) bool {
	var count int
	d.db.QueryRowContext(d.context,"SELECT COUNT(1) FROM USER_TAB_COLUMNS WHERE TABLE_NAME = :1 AND COLUMN_NAME = :2", strings.ToUpper(tableName), strings.ToUpper(columnName)).Scan(&count)
	return count > 0
}

func (d oracle) CurrentDatabase() (name string) {
	d.db.QueryRowContext(d.context,"SELECT GLOBAL_NAME FROM GLOBAL_NAME").Scan(&name)
	return
}
...
func (o oracle)SetContext(context context.Context){
	o.context = context
}
