...
func (s commonDialect) HasIndex(tableName string, indexName string) bool {
	var count int
	s.db.QueryRowContext(s.context,"SELECT count(*) FROM INFORMATION_SCHEMA.STATISTICS WHERE table_schema = ? AND table_name = ? AND index_name = ?", s.CurrentDatabase(), tableName, indexName).Scan(&count)
	return count > 0
}

func (s commonDialect) RemoveIndex(tableName string, indexName string) error {
	_, err := s.db.ExecContext(s.context,fmt.Sprintf("DROP INDEX %v", indexName))
	return err
}
...
func (s commonDialect) HasTable(tableName string) bool {
	var count int
	s.db.QueryRowContext(s.context,"SELECT count(*) FROM INFORMATION_SCHEMA.TABLES WHERE table_schema = ? AND table_name = ?", s.CurrentDatabase(), tableName).Scan(&count)
	return count > 0
}

func (s commonDialect) HasColumn(tableName string, columnName string) bool {
	var count int
	s.db.QueryRowContext(s.context,"SELECT count(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE table_schema = ? AND table_name = ? AND column_name = ?", s.CurrentDatabase(), tableName, columnName).Scan(&count)
	return count > 0
}

func (s commonDialect) CurrentDatabase() (name string) {
	s.db.QueryRowContext(s.context,"SELECT DATABASE()").Scan(&name)
	return
}
