	...
	if relationship.Kind == "belongs_to" {
		...
		if len(values) > 0 {
			...
			if len(newPrimaryKeys) > 0 {
				sql := fmt.Sprintf("%v NOT IN (%v)", scope.ToQueryCondition(associationForeignDBNames), utils.ToQueryMarks(newPrimaryKeys))
				newDB = newDB.Where(sql, utils.ToQueryValues(newPrimaryKeys)...)
			}
		}
		...
		if relationship.Kind == "many_to_many" {
			...
			if sourcePrimaryKeys := scope.GetColumnAsArray(sourceForeignFieldNames, scope.Value); len(sourcePrimaryKeys) > 0 {
				newDB = newDB.Where(fmt.Sprintf("%v IN (%v)", scope.ToQueryCondition(relationship.ForeignDBNames), utils.ToQueryMarks(sourcePrimaryKeys)), utils.ToQueryValues(sourcePrimaryKeys)...)
...
func (association *Association) Delete(values ...interface{}) *Association {
	...
	if relationship.Kind == "many_to_many" {
		...
		sql := fmt.Sprintf("%v IN (%v)", scope.ToQueryCondition(relationship.AssociationForeignDBNames), utils.ToQueryMarks(deletingPrimaryKeys))
		newDB = newDB.Where(sql, utils.ToQueryValues(deletingPrimaryKeys)...)
		...
		if relationship.Kind == "belongs_to" {
			...
			newDB = newDB.Where(
				fmt.Sprintf("%v IN (%v)", scope.ToQueryCondition(relationship.ForeignDBNames), utils.ToQueryMarks(primaryKeys)),
				utils.ToQueryValues(primaryKeys)...,
			)
			...
			newDB = newDB.Where(
				fmt.Sprintf("%v IN (%v)", scope.ToQueryCondition(deletingResourcePrimaryDBNames), utils.ToQueryMarks(deletingPrimaryKeys)),
				utils.ToQueryValues(deletingPrimaryKeys)...,
			)
		...
		if field.Kind() == reflect.Slice {
			...
			for i := 0; i < field.Len(); i++ {
				...
				for _, pk := range deletingPrimaryKeys {
					if utils.EqualAsString(primaryKey, pk) {
						isDeleted = true
						break
					}
				}
...
func (association *Association) Count() int {
	...
	if relationship.Kind == "many_to_many" {
		...
		query = query.Where(
			fmt.Sprintf("%v IN (%v)", scope.ToQueryCondition(relationship.AssociationForeignDBNames), utils.ToQueryMarks(primaryKeys)),
			utils.ToQueryValues(primaryKeys)...,
		)
	}
