...
func (scope *Scope) IndirectValue() reflect.Value {
	return utils.Indirect(reflect.ValueOf(scope.Value))
}
	...
	if scope.fields == nil {
		...
		for _, structField := range scope.GetModelStruct().StructFields {
			if isStruct {
				...
				fields = append(fields, &Field{StructField: structField, Field: fieldValue, IsBlank: utils.IsBlank(fieldValue)})
			} else {
...
func (scope *Scope) AddToVars(value interface{}) string {
	...
	if expr, ok := value.(*utils.Expression); ok {
		exp := expr.Expr
...
func (scope *Scope) Begin() *Scope {
	if db, ok := scope.SQLDB().(sqlDb); ok {
		if tx, err := db.BeginTx(scope.Db.Context, nil); err == nil {
	...
	for _, arg := range args {
		...
		case reflect.Slice: // For where("id in (?)", []int64{1,2})
			if bytes, ok := arg.([]byte); ok {
				...
				str = strings.Replace(str, "?", scope.AddToVars(utils.Expr("NULL")), 1)
			}
		default:
...
func (scope *Scope) selectSQL() string {
	...
	for _, f := range scope.Fields() {
		cols = append(cols, fmt.Sprintf("%v.%s", scope.QuotedTableName(), f.DBName))
	}
...
func (scope *Scope) orderSQL() string {
	...
	for _, order := range scope.Search.orders {
		...
		} else if expr, ok := order.(*utils.Expression); ok {
			exp := expr.Expr
			for _, arg := range expr.Args {
				exp = strings.Replace(exp, "?", scope.AddToVars(arg), 1)
			}
	...
	case interface{}:
		...
		case reflect.Map:
			for _, key := range reflectValue.MapKeys() {
				attrs[utils.ToDBName(key.Interface().(string))] = reflectValue.MapIndex(key).Interface()
			}
		default:
...
func (scope *Scope) UpdatedAttrsWithValues(value interface{}) (results map[string]interface{}, hasUpdate bool) {
	...
	for key, value := range convertInterfaceToMap(value, true) {
		if field, ok := scope.FieldByName(key); ok && scope.ChangeableField(field) {
			if _, ok := value.(*utils.Expression); ok {
				hasUpdate = true
...
func (scope *Scope) GetColumnAsArray(columns []string, values ...interface{}) (results [][]interface{}) {
	for _, value := range values {
		indirectValue := utils.Indirect(reflect.ValueOf(value))

		switch indirectValue.Kind() {
		case reflect.Slice:
			for i := 0; i < indirectValue.Len(); i++ {
				var result []interface{}
				var object = utils.Indirect(indirectValue.Index(i))
				var hasValue = false
				for _, column := range columns {
					field := object.FieldByName(column)
					if hasValue || !utils.IsBlank(field) {
						hasValue = true
					}
...
func (scope *Scope) GetColumnAsScope(column string) *Scope {
	...
	case reflect.Slice:
		if fieldStruct, ok := scope.GetModelStruct().ModelType.FieldByName(column); ok {
			...
			for i := 0; i < indirectScopeValue.Len(); i++ {
				result := utils.Indirect(utils.Indirect(indirectScopeValue.Index(i)).FieldByName(column))
...
func (scope *Scope) ToQueryCondition(columns []string) string {
