...
func (s *ModelStruct) TableName(db *DB) string {
	if s.defaultTableName == "" && db != nil && s.ModelType != nil {
		...
		if tabler, ok := reflect.New(s.ModelType).Interface().(tabler); ok {
			...
			tableName := utils.ToDBName(s.ModelType.Name())
...
func getForeignField(column string, fields []*StructField) *StructField {
	for _, field := range fields {
		if field.Name == column || field.DBName == column || field.DBName == utils.ToDBName(column) {
			return field
		}
	}
	return nil
	...
	for i := 0; i < reflectType.NumField(); i++ {
		if fieldStruct := reflectType.Field(i); ast.IsExported(fieldStruct.Name) {
								...
								if many2many := field.TagSettings["MANY2MANY"]; many2many != "" {
									...
									for _, foreignKey := range foreignKeys {
										if foreignField := getForeignField(foreignKey, modelStruct.StructFields); foreignField != nil {
											...
											joinTableDBName := utils.ToDBName(reflectType.Name()) + "_" + foreignField.DBName
									...
									for _, name := range associationForeignKeys {
										if field, ok := toScope.FieldByName(name); ok {
											...
											joinTableDBName := utils.ToDBName(elemType.Name()) + "_" + field.DBName
			...
			if value, ok := field.TagSettings["COLUMN"]; ok {
				...
				field.DBName = utils.ToDBName(fieldStruct.Name)
			}
