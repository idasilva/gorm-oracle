...
func TestCreate(t *testing.T) {
	...
	now := time.Now()
	user := User{Name: "CreateUser", Age: 18, Birthday: &now, UserNum: 111, Latitude: float} //PasswordHash: []byte{'f', 'a', 'k', '4'} }
...
func TestCreateWithNoStdPrimaryKeyAndDefaultValues(t *testing.T) {
	if dialect := os.Getenv("GORM_DIALECT"); dialect == "oracle" {
		t.Skip("Skipping this because Oracle does not support empty primary key")
	}
...
func TestSelectWithCreate(t *testing.T) {
	if dialect := os.Getenv("GORM_DIALECT"); dialect == "oracle" {
		t.Skip("Skipping this because only oracle properly support auto_increment")
	}
