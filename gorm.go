...
//    import _ "github.com/leocomelli/gorm/dialects/mysql"
//    // import _ "github.com/leocomelli/gorm/dialects/postgres"
//    // import _ "github.com/leocomelli/gorm/dialects/sqlite"
//    // import _ "github.com/leocomelli/gorm/dialects/mssql"
func Open(ctx context.Context, dialect Dialect, args ...interface{}) (db *DB, err error) {
	if len(args) == 0 {
...
func (s *DB) WithContext(ctx context.Context) {
	s.Context = ctx
}
...
func (s *DB) SetLogger(log logger.LoggerIface) {
	s.logger = log
}
...
func (s *DB) QueryExpr() *utils.Expression {
	scope := s.NewScope(s.Value)
	scope.InstanceSet("skip_bindvar", true)
	scope.PrepareQuerySQL()

	return utils.Expr(scope.SQL, scope.SQLVars...)
}
...
func (s *DB) Update(attrs ...interface{}) *DB {
	return s.Updates(utils.ToSearchableMap(attrs...), true)
}
...
func (s *DB) UpdateColumn(attrs ...interface{}) *DB {
	return s.UpdateColumns(utils.ToSearchableMap(attrs...))
}
...
func (s *DB) Begin() *DB {
	...
	if db, ok := c.db.(sqlDb); ok && db != nil {
		tx, err := db.BeginTx(s.Context, nil)
...
func (s *DB) Preload(column string, conditions ...interface{}) *DB {
	return s.clone().search.PreloadSearch(column, conditions...).db
}
...
func (s *DB) AddError(err error) error {
	if err != nil {
		if err != ErrRecordNotFound {
			if s.logMode == 0 {
				go s.print(utils.FileWithLineNum(), err)
			} else {
				s.log(err)
			}
...
func (s *DB) log(v ...interface{}) {
	if s != nil && s.logMode == 2 {
		s.print(append([]interface{}{"log", utils.FileWithLineNum()}, v...)...)
	}
}

func (s *DB) slog(sql string, t time.Time, vars ...interface{}) {
	if s.logMode == 2 {
		s.print("sql", utils.FileWithLineNum(), utils.NowFunc().Sub(t), sql, vars, s.RowsAffected)
	}
}
